...
1. 체언, 용언 및 어간만을 수집한다. 수식언, 독립언, 관계언은 수집하지 않는다.
...
1. 파생어의 경우,
1-1. 접두사는 제거하지 않고 그대로 수집한다.
ex. 불평등(불-평등) → 불평등(不平等)
    반제국주의(반-제국주의) → 반제국주의(反帝國主義)
1-2. 들, 상(上), 적(的), 꼴, 끼리, 님, 당, 어치, 여, 쯤, 차(次), 하(下), 리(裡) 등의 접미사들은 제거하고 나머지를 남긴다. 이외에는 제거하지 않고 그대로 수집한다.
ex. 씨름꾼(씨름-꾼) → 씨름꾼
    사람들(사람-들) → 사람
...
2. 합성어와 명사구 중 무엇에 해당하는지 판별할 때 띄어쓰기 여부를 고려하지 않는다.

<용언>

1. 대원칙을 고려하면서 ‘-’ 없이 기본형으로 표기한다. 동사 및 형용사만을 수집하며, 서술격 조사를 수집하지 않는다.
